...
def interactive_fcs_plot(stack,nsum, parn=1, normsize=1, fig = None, 
    ...
    print('Testing')
    fittype=stack.fitting_parameters_dict['mtype']
    if type(fittype)==bytes:
        fittype = fittype.decode('utf-8')
    print(fittype)
    if fittype=="2D_2c":
        return interactive_fcs_plot_2components(stack, nsum,normsize=normsize,
                        fig=fig,vmax=vmax,vmin=vmin,chi_threshold=chi_threshold,
                        ith=ith,light_version=light_version,use_mask=use_mask)
    mutable_object = {}
...
def interactive_fcs_plot_2components(stack,nsum, normsize=1, fig = None, 
                    vmax = None, vmin = None, chi_threshold = None, 
                    ith = None, light_version = True,use_mask=False):
    """Single method for display of FCS results."""
    print('Display 2 components')
    mutable_object = {}
    ...
    dmap = stack.fit_results_dict[nsum][:,:,1].copy()
    dmap=dmap.astype(float)
    dmap[dmap<0] = np.nan
